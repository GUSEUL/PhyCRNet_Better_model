...
def train_clean_model():
    ...
    for epoch in range(config['num_epochs']):
        ...
        if avg_val_loss < best_val_loss:
            ...
            try:
                # Ensure directory exists for model save path
...
def save_training_visualization(model, val_loader, train_losses, val_losses, epoch, device):
    ...
    fig, axes_array = plt.subplots(3, 4, figsize=(16, 12))
    ...
    # Convert to explicit indexing to avoid linter issues
    for i, (field_name, cmap) in enumerate(zip(field_names, cmaps)):
        # Input
        im1 = axes_array[0, i].imshow(input_img[i], cmap=cmap, aspect='equal', origin='lower')  # type: ignore
        axes_array[0, i].set_title(f'{field_name} (Input)')  # type: ignore
        plt.colorbar(im1, ax=axes_array[0, i], shrink=0.8)  # type: ignore
        
        # Target
        im2 = axes_array[1, i].imshow(target_img[i], cmap=cmap, aspect='equal', origin='lower')  # type: ignore
        axes_array[1, i].set_title(f'{field_name} (Target)')  # type: ignore
        plt.colorbar(im2, ax=axes_array[1, i], shrink=0.8)  # type: ignore
        
        # Prediction
        im3 = axes_array[2, i].imshow(pred_img[i], cmap=cmap, aspect='equal', origin='lower')  # type: ignore
        axes_array[2, i].set_title(f'{field_name} (Prediction)')  # type: ignore
        plt.colorbar(im3, ax=axes_array[2, i], shrink=0.8)  # type: ignore
